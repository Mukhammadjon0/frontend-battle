...
/// userName
document.querySelector('#signup-stop-one .stop-one-name').addEventListener('input', (e) => {
    signUserName = e.target.value
})

/// userEmail
document.querySelector('#signup-stop-one .stop-one-email').addEventListener('input', (e) => {
    signUserEmail = e.target.value
})

/// password
document.querySelector('#signup-stop-two .step-two-password1').addEventListener('input', (e) => {
    signPassword = e.target.value
})

/// password verify
document.querySelector('#signup-stop-two .step-two-password2').addEventListener('input', (e) => {
...
signUpNextBtn.addEventListener('click', () => {
        ...
        if(localStorage.getItem('email') == signUserEmail) {
            ...
            Toastify({
                ...
                gravity: "top", 
                positionLeft: false,
                backgroundColor: "linear-gradient(-90deg, rgba(253,29,29,1) 50%, rgba(252,176,69,1) 100%)"
            }).showToast(); 
        }

       else if(!signUserName == '' && !signUserEmail == '' || !signUserName == ' ' && !signUserEmail == ' ') {
...
const signUpSuccesNavigate = () => {
    window.location.assign('http://127.0.0.1:5500/pages/test-game/dictionarygame.html')
}